...
p <- ggplot(subway.15, aes(x=reorder(역명, station.total), y=station.total)) +
geom_bar(stat = 'identity') 
...
ggplot(subway.15, aes(x=역명, y=station.total/1000)) +
geom_bar(stat = 'identity', fill='lightblue', color = 'black', width = 0.6) +
theme(axis.text.x=element_text(angle=45)) + 
scale_y_continuous(label = dollar_format(prefix = "", suffix=" 명")) +
xlab("지하철역") +
ylab("인원(천명)") +
...
subway.month.gubun <- subway %>% group_by(역명, 구분) %>% 
...
ggplot(subway.15.gubun, aes(x=역명, y=station.total/10000, fill=구분)) + 
geom_bar(stat = 'identity', position = 'dodge') + 
scale_y_continuous(label = dollar_format(prefix = "", suffix = "명")) +
theme(axis.text.x=element_text(angle=45)) +
ggtitle("상위 15정거장 지하철역") +
